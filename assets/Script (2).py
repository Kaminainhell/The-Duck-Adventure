...
tempo_final_ms  = 0   # preenchido ao entrar em resultados
pontuacao_total = 0   # preenchido ao entrar em resultados
total_inimigos  = len(Galinha_group) + len(Galo_group)

dialogo_linhas = [
    "Herói! Você derrotou todos os inimigos!",
    "Eu sabia que você conseguiria...",
    "O reino inteiro está em dívida com você.",
    "Que sua coragem seja lembrada para sempre!",
]
dialogo_index = 0

fonte_dialogo = pygame.font.SysFont(None, 34)
fonte_dialogo_p = pygame.font.SysFont(None, 26)


def desenhar_dialogo(surf, linha):
    box_w, box_h = largura - 80, 140
    box_x, box_y = 40, altura - box_h - 20
    # fundo semi-transparente
    s = pygame.Surface((box_w, box_h), pygame.SRCALPHA)
    s.fill((0, 0, 0, 200))
    surf.blit(s, (box_x, box_y))
    # borda dourada
    ...
    # dica
    dica = fonte_dialogo_p.render("[ ENTER ] Continuar", True, (180, 180, 180))
...
def desenhar_resultados(surf, pato, tempo_ms, pontos):
    ...
    tempo_str = f"Tempo:  {minutos:02d}:{seg_rest:02d}"
    t = fonte.render(tempo_str, True, (180, 220, 255))
...
while rodando:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if estado == "dialogo":
                if event.key in (pygame.K_RETURN, pygame.K_SPACE):
                    ...
                    if dialogo_index >= len(dialogo_linhas):
                        # calcula pontuação ao ir para resultados
            ...
            # --- Estado: jogando ---
                ...
                # interagir com a princesa (E)
                if event.key == pygame.K_e:
                    for princesa in Princesa_group:
                        if princesa.liberada and pato.rect.colliderect(princesa.rect.inflate(60, 60)):
                            estado = "dialogo"
                            dialogo_index = 0
    ...
    # --- Pular lógica de jogo se não estiver jogando ---
    if estado == "dialogo":
        # renderiza o cenário congelado por baixo
        tela.fill((0, 0, 0))
